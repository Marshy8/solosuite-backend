...
describe("createBooking", () => {
  it("creates an event with no description for a brand-new client", async () => {
    ...
    await createBooking({
      ...
      service_type_id: 1, // Buzz Cut, 15 min
  ...
  it("enriches the description with the client's last visit when a note exists", async () => {
    ...
    await createBooking({
      client_identifier: "555-0101", // seeded: Jordan Lee, last_service_type_id 2
  ...
  it("computes the end time from the requested service's duration", async () => {
    createEventMock.mockResolvedValue({ id: "evt_3" });

    await createBooking({
      client_identifier: "555-0301",
      client_name: "New Client",
      service_type_id: 5, // Hot Towel Shave, 40 min
    ...
    expect(createEventMock).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        end: { dateTime: "2026-09-01T10:40:00.000Z" },
      }),
    );
  });
